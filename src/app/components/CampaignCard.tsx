...
export default function CampaignCard({ campaignAddress }: CampaignCardProps ) {
    ...
    let balancePorcentage = (parseInt(totalBalance as string) / parseInt(totalGoal as string)) * 100; 

    if(balancePorcentage >= 100) {
        balancePorcentage = 100;
    }
    ...
    return (
        <div className="flex flex-col justify-between max-w-sm p-6 bg-white border border-slate-200 rounded-lg shadow">
            ...
            {!isPendingBalance && (
                        <div className="mb-4">
                            <div className="relative w-full h-6 bg-gray-200 rounded-full dark:bg-gray-700">
                                <div className="h-6 bg-blue-600 rounded-full dark:bg-blue-500 text-right" style={{ width: `${balancePorcentage?.toString()}%`}}>
                                    <p className="text-white dark:text-white text-xs p-1">${balance?.toString()}</p>
                                </div>
                                <p className="absolute top-0 right-0 text-white dark:text-white text-xs p-1">
                                    {balancePorcentage >= 100 ? "" : `${balancePorcentage?.toString()}%`}
                                </p>
                            </div>
                        </div>
                )}
            ...
            <Link 
                ...
                passHref={true}
            >
                <p className="inline-flex items-center px-3 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                    View Campaign
